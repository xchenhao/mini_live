var JsInterface={
	...
	chatFromSocket: function(data) {
		...
		}else if(msgmethod=='ConnectVideo'){ //连麦
	...
	connectVideo:function(data){
		...
		if(action==1){
			/* 请求连麦 */
			if(uid==liveuid){
				var l_t=setTimeout("layer.closeAll()",10000);
				layer.closeAll();
				layer.confirm(data.uname+'向您发起连麦请求', {
					btn: ['接受','拒绝'] //按钮
				}, function(){
					layer.closeAll();
					Linkmic.linkmic_allow(data.touid);
				}, function()
				{
					layer.closeAll();
					Linkmic.linkmic_refuse(data.touid);
				});
			}
