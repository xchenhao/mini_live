...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		//备案信息
		$site_icp=$config['site_icp'];
		$this->assign("site_icp",$site_icp);
		//站点地址
		$site_addr=$config['site_addr'];
		$this->assign("site_addr",$site_addr);
		//站点电话
		$site_telephone=$config['site_telephone'];
		$this->assign("site_telephone",$site_telephone);
		//站点邮箱
		...
		//开播时间
		$live_show_time=$config['live_show_time'];
		$this->assign("live_show_time",$live_show_time);
		//直播用户
		$config=M("config")->where("id='1'")->find();
		/* 才艺直播 */
